...
def generate_dataset(image_set, input_path, features, output_path):
    ...
    for image in image_set.images:
        if os.path.basename(image.metadata.image_path) not in features:
            continue
        new_image = new_image_set.images.add()
        new_image.CopyFrom(image)
        shutil.copy(input_path + "/" + os.path.basename(image.metadata.image_path), output_path + "/" + os.path.basename(image.metadata.image_path))
...
def main():
    ...
    rois = roi_metadata.read_imageset_file(args.input_path)
