...
def crop(image_proto, metadata_list):
    """
    ...
    image.metadata.image_path = "crop_20_" + os.path.basename(image_proto.metadata.image_path)
...
def slide_window(image_proto, image_size, metadata_list):
    """
    ...
    image.metadata.image_path = "slide_" + os.path.basename(image_proto.metadata.image_path)
    for roi in image_proto.rois:
        roi_new = image.rois.add()
        roi_new.CopyFrom(roi)
...
def rotate_ccw(image, image_size, rotation_matrix, metadata_list):
    ...
    new_image.metadata.image_path = "rotate_ccw_" + os.path.basename(image.metadata.image_path)
...
def rotate_cw(image, image_size, rotation_matrix, metadata_list):
    ...
    height, width, _ = image_size
    new_image = create_image_proto()
    new_image.metadata.image_path = "rotate_cw_" + os.path.basename(image.metadata.image_path)
    for roi in image.rois:
        points_to_transform = np.array([
            [roi.rect.tl.col, roi.rect.tl.row, 1],
            [roi.rect.br.col, roi.rect.br.row, 1],
...
def darken_image(image, output_metadata):
    ...
    """
    darken_image_file = "darken_image_" + os.path.basename(image.metadata.image_path)
...
def brighten_image(image, output_metadata):
    ...
    """
    brighten_image_file = "brighten_image_" + os.path.basename(image.metadata.image_path)
...
def paralel_augment(image_proto, in_path, out_path):
    ...
    output_metadata = []
    image_path = in_path + "/" + os.path.basename(image_proto.metadata.image_path)
    ...
    darken_value = randint(80, 98) / 100
    darken_image_data = image_operations.adjust_gamma(image_data, darken_value)
    ...
    brighten_value = randint(102, 120) / 100
    brighten_image_data = image_operations.adjust_gamma(image_data, brighten_value)
...
def main():
    ...
    metadata = proto_api.read_imageset_file(args.input_path)
    output_metadata = augment(
        metadata, os.path.dirname(args.input_path), threads_number, args.output_path)
    output_metadata.name = ""
    proto_api.serialize_proto_instance(output_metadata, args.output_path)
