...
class DS_MQ_Param(MQ_Param):
    ...
    ACCUMULATED_PROTO_SIZE = "accumulated_proto_size"
...
class DiskSerializer(AbstractMQConsumer):
    ...
    def __init__(self, config, mq_message_type: MQ_Messsage_Type = MQ_Messsage_Type.IMAGE, **kwargs):
        ...
        self.timeout_period = int(config_api.get_config_param(DS_MQ_Param.TIMEOUT_PERIOD, config, 60))
        self.accumulated_proto_size = int(config_api.get_config_param(DS_MQ_Param.ACCUMULATED_PROTO_SIZE, config, 0))
    ...
    def __check_valid_config(self):
        ...
        for key in [DS_MQ_Param.INPUT_PROTO_TYPE, DS_MQ_Param.OUTPUT_PATH, DS_MQ_Param.TIMEOUT_PERIOD,
                    DS_MQ_Param.ACCUMULATED_PROTO_SIZE]:
    ...
    def __timeout_thread(self):
        while True:
            ...
            proto_count = len(self.accumulated_protos)
            logger.info(f"Waiting for {delta_seconds}s or until {self.accumulated_proto_size} proto images.")
            if 0 < self.accumulated_proto_size <= proto_count:
                logger.info("Accumulated proto size exceeded. Writing to disk...")
                self.__flush_proto_to_disk()
            elif delta_seconds > self.timeout_period:
                if proto_count == 0:
                    logger.info("Timeout exceeded. No protos accumulated...")
                else:
                    logger.info("Timeout exceeded. Writing to disk...")
                    self.__flush_proto_to_disk()
                    self.mq_provider.stop_consuming_all()
                break
