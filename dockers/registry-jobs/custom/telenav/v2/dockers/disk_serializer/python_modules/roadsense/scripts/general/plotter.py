...
def plot_windows(clustered_df_original, centroid_df, lat_col, lon_col, nr_items=None, with_centroid=True):
    ...
    for _, row in tqdm(clustered_df[:nr_items].iterrows()):
        ...
        link = "http://www.openstreetcam.org/details/{}/{}/".format(trip_id, int(row[Column.IMAGE_INDEX]))
                               
        popup_text = '<a href="' + link + '"target="_blank">' + link + '</a> </br>'  
    ...
    folium.TileLayer('cartodbpositron').add_to(view_map)
...
def plot_meta_clusters(meta_clustered_df, meta_centroid_df, nr_items=None, is_hazard=False):
    ...
    for _, row in tqdm(meta_clustered_df[:nr_items].iterrows()):
        ...
        member_ids = meta_centroid_df[meta_centroid_df[Column.META_CLUSTER_ID] == row["meta_cluster_id"]][Column.MEMBER_IDS].values[0]
        seen_trips = set()
        for trip,index in member_ids:
            if trip in seen_trips:
                continue
            link = f"http://beta.openstreetcam.org/details/{trip}/{index}/track-info"
            popup_text += '<a href="' + link + '"target="_blank">' + f"Trip {trip} Image {index}" + '</a> </br>'  
            seen_trips.add(trip)
        centroid_point = folium.Circle([centroid_lat, centroid_lon], radius=10,popup = folium.Popup(popup_text),
