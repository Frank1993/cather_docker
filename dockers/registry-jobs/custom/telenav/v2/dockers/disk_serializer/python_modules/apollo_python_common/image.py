...
def get_bgr(image_url, osc_details=None):
    ...
    if _url_resource_exists(image_url, parsed_url.scheme):
        ...
        if osc_details is None:
            ...
            new_image_url = get_image_path(osc_details.image_id, osc_details.osc_api_url)
...
def get_image_path(image_id, osc_api_url):
...
def crop_square_roi(img, xmin, ymin, xmax, ymax, sq_crop_factor):
    """ Given a full image, the coordinates of a crop from the image and a square crop factor, returns a square crop from
     the image, using the initial set of coordinates and the crop factor to make the returned crop square. """

    roi_rect = Rectangle(xmin, ymin, xmax, ymax)
    img_rect = Rectangle(0, 0, img.shape[1], img.shape[0])

    square = (roi_rect * sq_crop_factor).clip_in_rect(img_rect).expand_to_square(img_rect)

    return img[square.ymin:square.ymax, square.xmin:square.xmax]
