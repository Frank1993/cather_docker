...
class OCRPredictor:
    ...
    def __init__(self, dataset_name, ckpt_path, text_corrector, custom_charset_path = None):
        self.width, self.height = self.__get_dataset_image_size(dataset_name)
        self.null_char = self.__get_null_char(dataset_name,custom_charset_path)
        self.text_corrector = text_corrector

        self.imgs_placeholder, self.endpoints = self.__get_model(dataset_name, custom_charset_path)
    ...
    def __create_dataset(self, dataset_name, dataset_dir, split_name):
        ds_module = getattr(datasets, dataset_name)
        return ds_module.get_split(split_name, dataset_dir=dataset_dir)

    def __load_model(self, dataset_name, custom_charset_path):
        dataset = self.__create_dataset(dataset_name, dataset_dir = custom_charset_path, split_name="train")
    ...
    def __get_null_char(self, dataset_name, custom_charset_path):
        ds_module = getattr(datasets, dataset_name)
        null_code = ds_module.DEFAULT_CONFIG[self.NULL_CODE_KEY]
        charset_name = ds_module.DEFAULT_CONFIG[self.CHARSET_FILENAME_KEY]
        charset_path = ds_module.DEFAULT_DATASET_DIR if custom_charset_path is None else custom_charset_path
        charset_full_path = os.path.join(charset_path, charset_name)
        charset = ds_module.read_charset(charset_full_path)
    ...
    def __get_model(self, dataset_name, custom_charset_path):
        model = self.__load_model(dataset_name, custom_charset_path)
