...
class PoiTextGenerator(AbstractTextGenerator):
    CITY_KEY = "city"
    UPPERCASE_PROB = 0.3
    SLASH_PROB = 0.5
    WORD_CHANGE_DICT = {"Road": "Rd", "Street": "St", "Avenue": "Ave",
                        "Drive": "Dr", "Boulevard": "Blvd", "Freeway": "Fwy"}

    def __init__(self, resources_path, nr_texts_per_length, char_limit):
    ...
    def __change_word(self, word):
        return self.WORD_CHANGE_DICT[word] if word in self.WORD_CHANGE_DICT else word
    ...
    def generate_texts(self):
        ...
        for text_length in tqdm(range(2, self.char_limit + 1)):

            random_texts = [text for text in texts if text_length == len(text)]
            random_texts = [self.__randomize(text) for text in random_texts]

            if len(random_texts) < self.nr_texts_per_length:
                scale_factor = (self.nr_texts_per_length // len(random_texts)) + 1
            ...
            final_texts += random_texts[:self.nr_texts_per_length]
