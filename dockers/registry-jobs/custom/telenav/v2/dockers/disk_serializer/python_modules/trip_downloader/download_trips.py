    ...
    def __download_photo(self, image_data):
        ...
        try:
            ...
            request = requests.get(image_url, auth=authentication, stream=True, allow_redirects=True,
                                   timeout=OSCDownloader.REQUEST_TIMEOUT_SEC)
...
def download_images(df, download_path, config, threads_number):
    ...
    if config.full_trips:
        for seq_id in config.trip_ids_included:
            if int(seq_id) not in trips_with_images.keys():
                trip_images = get_osc_trip_images(seq_id)
                osc_images.extend(trip_images)

    osc_images_df = pd.DataFrame(osc_images, columns=[SEQUENCE_ID, IMAGE_INDEX, FILE_PATH, SHOT_DATE])

    osc_images_df[[SEQUENCE_ID, IMAGE_INDEX]] = osc_images_df[[SEQUENCE_ID, IMAGE_INDEX]].astype(int)
    df[[SEQUENCE_ID, IMAGE_INDEX]] = df[[SEQUENCE_ID, IMAGE_INDEX]].astype(int)
...
def add_rois_to_image_proto(image_proto, rois_df, im_width, im_height):
    ...
    for _, row in sign_rois_df.iterrows():
        ...
        roi.local.distance = float(row[DISTANCE])
        roi.local.angle_of_roi = float(row[ANGLE_OF_ROI])
        roi.local.angle_from_center = 0
        roi.local.position.longitude = float(row[ROI_LONGITUDE])
        roi.local.position.latitude = float(row[ROI_LATITUDE])
        roi.local.facing = float(row[FACING])
...
def create_proto_output(detections_df, empty_images_df, config, download_path, fail_to_download_images):
    ...
    for image_fields, roi_df in tqdm.tqdm(grouped):
        seq_id, seq_index, lon, lat, im_width, im_height, heading, osc_id, gps_acc, region, file_path, shot_date = image_fields
        ...
        image_proto.sensor_data.gps_accuracy = gps_acc
        ...
        image_proto.metadata.id = str(osc_id)
