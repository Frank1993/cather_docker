...
from roadsense.scripts.general.config import ConfigParams as cp, Column, FolderName as fn
...
class HazardDatasetBuilder:
    ...
    def _handle_location_cols(self, df):
        df[Column.HAZARD_LAT] = df[Column.LOCATION].apply(lambda l: l.split(",")[0])
        df[Column.HAZARD_LON] = df[Column.LOCATION].apply(lambda l: l.split(",")[1])
        return df.drop(Column.LOCATION, axis=1)

    def _read_hazard_df(self, hazard_folder, specific_hazards=None):
    ...
    def _add_datetime(self, hazard_df):
        hazard_df[Column.DATETIME] = pd.to_datetime(hazard_df[Column.TIMESTAMP], unit='ms')
        hazard_df[Column.DATETIME] = hazard_df[Column.DATETIME].dt.round("{}L".format(self.config[cp.FREQUENCY]))
    ...
    def get_hazard_df(self, drive_folder):
        ...
        hazard_df = self._read_hazard_df(hazard_folder, self.config[cp.SPECIFIC_HAZARDS])
        hazard_df = self._handle_location_cols(hazard_df)
        hazard_df = self._add_datetime(hazard_df)
        return hazard_df
