...
from apollo_python_common import proto_api
...
flags.DEFINE_integer('num_rois', 0, 'Number of ROIs in each batch.')
...
def make_roi_dataframe(rois_file):
    ...
    image_set = proto_api.read_imageset_file(rois_file)

    data_dict = {'image': [], 'tl_row': [], 'tl_col': [], 'br_row': [], 'br_col': [], 'roi_class': []}

    for img_proto in tqdm(image_set.images):
        for roi in img_proto.rois:
            data_dict['image'].append(os.path.basename(img_proto.metadata.image_path))
...
def make_roi_sample_batches(roi_df, num_batches, num_rois, output_dir):
    ...
    num_classes = roi_df['roi_class'].nunique()
...
def main(_):
    ...
    if FLAGS.excl_samples_dir:
        ...
        if FLAGS.csv_file:
            roi_df = load_roi_dataframe(FLAGS.csv_file)
        else:
    ...
    make_roi_sample_batches(roi_df, FLAGS.num_batches, num_rois, FLAGS.output_dir)
