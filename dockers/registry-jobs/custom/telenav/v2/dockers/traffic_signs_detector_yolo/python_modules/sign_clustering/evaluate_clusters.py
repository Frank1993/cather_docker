...
def draw_gt_clusters(df):
    ...
    for cluster_id, rois_df in grouped:
        idx_2_color = {i: get_random_color() for i in set(rois_df[PREDICTED_CLUSTER_LABEL].values)}
        map_osm = folium.Map()
        for _, row in rois_df.iterrows():
            lat = row[LATITUDE]
            long = row[LONGITUDE]
            pred_cluster_id = int(row[PREDICTED_CLUSTER_LABEL])
            heading = row[HEADING]
            url = row["url"]
            roi_type = row[TYPE]
            roi_x = row[ROI_X]
        ...
        cluster_csv_file = "cluster_{}.csv".format(cluster_id)
        cluster_html_file = "cluster_{}.html".format(cluster_id)
        map_osm.fit_bounds(map_osm.get_bounds())
        map_osm.save(cluster_html_file)
        rois_df.to_csv(cluster_csv_file)
...
def compare_clusters_to_gt(clusters_df):
    grouped_predicted_clusters = clusters_df.groupby([PREDICTED_CLUSTER_LABEL])
    grouped_predicted_clusters = sorted(list(grouped_predicted_clusters), key=lambda tuple: -len(tuple[1]))
    grouped_gt = clusters_df.groupby([GT_CLUSTER_ID])
    ...
    for pred_cluster_id, pred_roi_ids_df in grouped_predicted_clusters:
    ...
    return nr_rois_in_correct_clusters / (len(clusters_df) - nr_invalid_cluster_rois), \
...
if __name__ == "__main__":
    ...
    #draw_gt_clusters(clustered_df)
    #draw_predicted_clusters(clustered_df)
