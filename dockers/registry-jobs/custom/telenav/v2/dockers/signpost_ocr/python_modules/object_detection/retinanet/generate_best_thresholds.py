...
class BestThresholdEvaluator:
    ...
    def __init__(self, gt_dict, pred_dict, min_size, is_for_signposts=False):
        self.gt_dict = gt_dict
        self.pred_dict = pred_dict
        self.min_size = min_size
        self.is_for_signposts = is_for_signposts

    def __compute_statistics(self, target_gt_dict, target_pred_dict):
        model_statistics = ModelStatistics(proto_eval.convert_rois_dictionary(target_gt_dict, self.is_for_signposts),
                                           proto_eval.convert_rois_dictionary(target_pred_dict, self.is_for_signposts),
                                           self.min_size)
        model_statistics.compute_model_statistics()
        return model_statistics.statistics

    def __filter_by_class(self, d, class_name):
        return proto_api.filter_rois_by_classes([class_name], d, self.is_for_signposts)

    def __filter_by_confidence(self, d, class_name, thr):
        return proto_api.get_confident_rois(d, {class_name: thr}, self.is_for_signposts)
    ...
    def __get_all_classes(self):
        all_classes = set(proto_api.get_class_names_from_images_dictionary(self.gt_dict, self.is_for_signposts)) \
            .union(set(proto_api.get_class_names_from_images_dictionary(self.pred_dict, self.is_for_signposts)))
        return all_classes
    ...
    def evaluate_with_best_thresholds(self, thresholds_per_class):
        confident_dict = proto_api.get_confident_rois(self.pred_dict,
                                                      thresholds_per_class,
                                                      self.is_for_signposts)
...
def parse_args(args):
    ...
    parser.add_argument("-s", "--is_for_signposts",
...
def main():
    ...
    gt_dict, pred_dict = \
        proto_eval.get_proto_dict(args.gt_rois_file,
                                  args.pred_rois_file,
                                  args.selected_classes_file,
                                  None, args.is_for_signposts)

    thresh_eval = BestThresholdEvaluator(gt_dict, pred_dict,
                                         min_size=args.min_size,
                                         is_for_signposts=args.is_for_signposts)
