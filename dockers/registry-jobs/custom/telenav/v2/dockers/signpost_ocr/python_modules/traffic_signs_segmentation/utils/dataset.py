...
def no_false_positives(roi):
    ...
    """
    return roi.validation == orbb_definitions_pb2.FALSE_POSITIVE
...
def load_all_rois(input_path, roi_filter=dummy_roi_filter):
    ...
    for image in metadata.images:
        current_image = os.path.normpath(
            os.path.dirname(input_path) + "/" + os.path.basename(image.metadata.image_path))
...
def load_valid_rois(input_path):
    ...
    for image_rois in metadata.images:
        current_image = os.path.dirname(input_path) + "/" + os.path.basename(image_rois.metadata.image_path)
        if not os.path.exists(current_image):
            continue
        ...
        if not image.valid_image(width, height, conf.ratio, conf.epsilon):
            ...
            logger.info("invalid image because aspect ratio {} {} {}".format(width, height, os.path.basename(image.metadata.image_path)))
            continue
...
class Roi:
    ...
    def __init__(self, orbb_roi):
        ...
        self.false_pos = (orbb_roi.validation
                          == orbb_definitions_pb2.FALSE_POSITIVE)
...
class FeatureGenerator:
    ...
    def __call__(self, image_rois):
        ...
        # TODO: cv2
             try:
                img = image.load_image(image_rois[0])
                if img is not None:
                    img_path = os.path.join(self.features_path,
                                            os.path.basename(image_rois[0]))
                    resized_img, width_crop_size, height_crop_size = image.resize_image_fill(img, conf.image_size[1], conf.image_size[0], 3)
                ...
                resize_label_image, _, _ = image.resize_image_fill(label, conf.image_size[1], conf.image_size[0], 0, 'nearest' )

                label_img_pil = Image.fromarray(resize_label_image, 'P')
                label_img_pil.putpalette(conf.palette)

                label_path = self.labels_path + os.path.basename(utils.to_png(image_rois[0]))
                label_img_pil.save(label_path)
                ...
                with open(label_file_path, "a+") as label_file:
                  ...
                  logger = logging.getLogger(__name__)
                  logger.error("Feature Generator Error For Image : {} ".format(image_rois[0]))
