...
def __parse_args(args):
    ...
    parser.add_argument('--cut_below_vanishing_point',
                        ...
                        type=int)
    parser.add_argument('--is_for_signposts',
                        help="Whether will predict signpost components",
...
def main():
    ...
    if args.is_for_signposts:
        rois_labels = SignComponentsTypes(args.train_meta_file)
    else:
        rois_labels = TrafficSignsTypes(args.train_meta_file)

    if args.threshold_file == 'SAME':
        score_threshold_per_class = dict([(class_label, args.lowest_score_threshold) for class_label in rois_labels.captions()])
    else:
    ...
    predict_one_folder(args.input_images_path, args.output_images_path, rois_labels, model,
                       score_threshold_per_class, resolutions,
                       cut_below_vanishing_point=args.cut_below_vanishing_point==1)
