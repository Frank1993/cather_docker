...
import classification.fast_ai.common.data
...
import classification.fast_ai.signs_facing_classifier.utils.data_common as data_common
...
class SignFacingDatasetMatcher:
    ...
    def _get_preds_by_class_and_img(class_name, img_name, pred_df):
        ...
        return pred_df[
            (pred_df[RoiDfColumn.IMG_NAME_COL] == img_name) & (pred_df[RoiDfColumn.ROI_CLASS_COL] == class_name)]
    ...
    def _save_crop(self, row):
        ...
        new_imgs, old_imgs = data_common.get_orig_images(self.new_orig_img_dir, self.old_orig_img_dir)
        img_path = data_common.get_orig_img_path(row[RoiDfColumn.IMG_NAME_COL], new_imgs, old_imgs,
                                                 self.new_orig_img_dir, self.old_orig_img_dir)
    ...
    def split_and_save(self):
        ...
        train_df, test_df = classification.fast_ai.common.data.train_test_split_stratified(self.gt_df,
        ...
        io_utils.create_folder(os.path.join(self.test_set_dir, SignFacingLabel.FRONT))
        io_utils.create_folder(os.path.join(self.test_set_dir, SignFacingLabel.LEFT))
        io_utils.create_folder(os.path.join(self.test_set_dir, SignFacingLabel.RIGHT))

        io_utils.create_folder(os.path.join(self.train_set_dir, SignFacingLabel.FRONT))
        io_utils.create_folder(os.path.join(self.train_set_dir, SignFacingLabel.LEFT))
        io_utils.create_folder(os.path.join(self.train_set_dir, SignFacingLabel.RIGHT))
