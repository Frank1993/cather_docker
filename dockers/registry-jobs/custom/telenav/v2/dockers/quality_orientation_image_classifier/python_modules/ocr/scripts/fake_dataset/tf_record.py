...
class TFRecordGenerator:
    ...
    def create_tf_records(self, imgs_save_path, tf_data_save_path):
        
        old_charset_path = os.path.join(self.config.resources_path,self.CHARSET_FILE_NAME)
        new_charset_path = os.path.join(tf_data_save_path,self.CHARSET_FILE_NAME)

        io_utils.create_folder(tf_data_save_path)
        shutil.copyfile(old_charset_path, new_charset_path)

        train_input_path = os.path.join(imgs_save_path, self.TRAIN_FOLDER)
        train_output_path = os.path.join(tf_data_save_path, self.TRAIN_FOLDER)

        test_input_path = os.path.join(imgs_save_path, self.TEST_FOLDER)
        test_output_path = os.path.join(tf_data_save_path, self.TEST_FOLDER)
        
        
        TFRecordDatasetCreator(train_input_path, train_output_path, 
                               new_charset_path, self.TRAIN_PATTERN, self.config.max_char_length).create_dataset()

        TFRecordDatasetCreator(test_input_path, test_output_path, 
                               new_charset_path, self.TEST_PATTERN, self.config.max_char_length).create_dataset()
...
class TFRecordDatasetCreator:
    ...
    def __init__(self, input_path, output_path, charset_path, name_pattern, max_char_length):
    ...
    def __encode_utf8_string(self,text):
        ...
        for _ in range(len(text),self.max_char_length):
