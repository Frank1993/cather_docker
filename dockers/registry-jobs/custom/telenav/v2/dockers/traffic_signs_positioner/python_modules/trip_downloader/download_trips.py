...
def create_proto_output(detections_df, empty_images_df, config, download_path, fail_to_download_images):
    ...
    detections_df[OSC_ID].fillna(-1, inplace=True)
    empty_images_df[OSC_ID].fillna(-1, inplace=True)

    grouped = detections_df.groupby(IMAGE_FIELDS)
    logger.info("Writing proto files")
    for (seq_id, seq_index, lon, lat, im_width, im_height, heading, osc_id, region, file_path, shot_date), roi_df in tqdm.tqdm(grouped):
