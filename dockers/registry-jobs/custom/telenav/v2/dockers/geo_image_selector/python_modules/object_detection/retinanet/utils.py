...
import object_detection.keras_retinanet.utils.image as retinanet_image
...
def predict_folder(model, input_folder, output_folder, resolutions, rois_labels, score_threshold_per_class,
                   algorithm="retinanet",draw_predictions=False, max_number_of_images=None,
                   cut_below_vanishing_point=True, log_level=0):
    ...
    for idx, file_name in enumerate(tqdm(img_files_lst)):
        ...
        try:
            ...
            if image is not None:
                image, image_pred = preprocess_image(image, cut_below_vanishing_point)
    ...
    metadata = get_preds_in_common_format(all_predictions, algorithm, "", rois_labels)
    return metadata


def preprocess_image(image, cut_below_vanishing_point):
    if cut_below_vanishing_point:
...
def get_predicted_detections(model, images_for_pred):
    if type(model) is Keras_Model:
        boxes, scores, labels = model.predict(images_for_pred)
        ...
        boxes_t = session.graph.get_tensor_by_name("{}/output0:0".format(graph_name))
        scores_t = session.graph.get_tensor_by_name("{}/output1:0".format(graph_name))
        labels_t = session.graph.get_tensor_by_name("{}/output2:0".format(graph_name))
        boxes, scores, labels = session.run([boxes_t, scores_t, labels_t], feed_dict={input_tensor: images_for_pred})
    else:
    ...
    for idx_img in range(len(images_for_pred)):
        ...
        indices = np.where(scores[idx_img, :] > MIN_SCORE_THRESHOLD)[0]
        # select those scores
        scores = scores[idx_img][indices]
        # find the order with which to sort the scores
        scores_sort = np.argsort(-scores)[:MAX_DETECTIONS]
        # select detections
        image_boxes      = boxes[idx_img, indices[scores_sort], :]
        image_scores     = scores[scores_sort]
        image_labels     = labels[idx_img, indices[scores_sort]]
...
def predict_images_on_batch(images, model, rois_labels, resolutions, score_threshold_per_class, log_level):
    all_boxes, all_resolutions, all_predicted_labels, all_scores, all_predicted_captions = \
    ...
    for resolution in resolutions:
        ...
        for i in range(len(images)):
            ...
            predicted_captions = [rois_labels.label_to_caption(label) for label in predicted_labels]
            ...
            all_predicted_captions.setdefault(i, []).append(predicted_captions)
    ...
    for i in range(len(images)):
        ...
        all_predicted_captions_ar = np.concatenate(all_predicted_captions[i])
        # Non_max_suppression
        selected_indices = non_max_suppression(all_boxes_ar, all_scores_ar, all_resolutions_ar,
                                               all_predicted_captions_ar, score_threshold_per_class)
        ...
        selected_predicted_captions = all_predicted_captions_ar[selected_indices]
        final_predictions.append((selected_boxes, selected_scores, selected_predicted_captions))
...
def predict_one_image(image_pred, model, rois_labels, resolutions, score_threshold_per_class, log_level):
    ...
    for resolution in resolutions:
        ...
        boxes = boxes / scale
        resolution = np.full(len(boxes), resolution)
        predicted_label_names = [rois_labels.label_to_caption(label) for label in predicted_labels]
...
def get_preds_in_common_format(all_predictions, algorithm, algorithm_version, rois_labels):
    metadata = meta.get_new_imageset_proto()
    ...
    for file_name, (boxes, scores, label_names) in all_predictions.items():
        ...
        image_proto.metadata.id = ""
        add_detections_to_img_proto(algorithm, algorithm_version, image_proto, boxes, scores, label_names,
                                    0, rois_labels)
    return metadata


def add_detections_to_img_proto(algorithm, algorithm_version, image_proto, boxes, scores, label_names,
                                min_side_size, rois_labels=None):
    for box, score, label_name in zip(boxes, scores, label_names):
        ...
        if (br_col - tl_col) >= min_side_size and (br_row - tl_row) >= min_side_size:
            add_roi_to_img_proto(image_proto, algorithm, algorithm_version, label_name, score,
                                 tl_row, tl_col, br_col, br_row, rois_labels)
...
def add_roi_to_img_proto(image_proto, algorithm, algorithm_version, label_name, score, tl_row, tl_col, br_col, br_row,
    ...
    if rois_labels is None:
        roi.type = meta.get_roi_type_value(label_name)
    else:
        roi.type = rois_labels.caption_to_name(label_name)
...
def non_max_suppression(boxes, scores, resolutions, predicted_captions, score_threshold_per_class,
                        iou_threshold=0.5):
    ...
    pick = set([i for i in range(len(boxes)) if scores[i] > score_threshold_per_class[predicted_captions[i]]])
    ...
    while should_continue:
        ...
        for (i1, i2) in itertools.combinations(list(range(0, len(boxes))), 2):
            if i1 in pick and i2 in pick:
                r1 = Rectangle(x1[i1], y1[i1], x2[i1], y2[i1])
                r2 = Rectangle(x1[i2], y1[i2], x2[i2], y2[i2])
                score1 = scores[i1]
                score2 = scores[i2]
                rect_intersect_area = r1.get_intersection_area(r2)
                if rect_intersect_area / r1.area() > iou_threshold or rect_intersect_area / r2.area() > iou_threshold:
