...
def convert_rois_dictionary(rois_dictionary, is_for_signposts=False):
    dictionary = dict()
    roi_count = 0
    for file_name in rois_dictionary.keys():
        ...
        for roi in rois_list:
            ...
            new_detection = ModelStatistics.create_detection(roi_count, _get_roi_caption(roi.type, is_for_signposts),
...
def _get_roi_caption(roi_type, is_for_signposts):
    if is_for_signposts:
        caption = proto_api.get_component_type_name(roi_type)
    else:
        caption = proto_api.get_roi_type_name(roi_type)
    return caption


def get_data_dicts(expected_rois_file, actual_rois_file, selected_classes_file,
                   classes_thresholds_file, is_for_signposts):

    expected_proto_dictionary, actual_proto_dictionary = get_proto_dict(expected_rois_file, actual_rois_file,
                                                                        selected_classes_file,
                                                                        classes_thresholds_file, is_for_signposts)

    expected_detection_dictionary = convert_rois_dictionary(expected_proto_dictionary, is_for_signposts)
    actual_detection_dictionary = convert_rois_dictionary(actual_proto_dictionary, is_for_signposts)

    return expected_detection_dictionary, actual_detection_dictionary


def get_proto_dict(expected_rois_file, actual_rois_file,
                   selected_classes_file, classes_thresholds_file, is_for_signposts):
    ...
    if classes_thresholds_file:
        ...
        actual_proto_dictionary = proto_api.get_confident_rois(actual_proto_dictionary,
                                                               classes_thresholds, is_for_signposts)

    return expected_proto_dictionary, actual_proto_dictionary
...
def main():
    ...
    parser.add_argument("-sp", "--is_for_signposts", help="Whether will evaluate signposts",
