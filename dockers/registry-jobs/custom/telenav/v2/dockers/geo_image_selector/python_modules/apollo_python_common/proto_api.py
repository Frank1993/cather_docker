...
def get_classes_values_from_images_dictionary(rois_dict):
    all_classes = set()
    for file_name, rois in rois_dict.items():
        for roi in rois:
            all_classes.add(roi.type)
    return sorted(list(all_classes))
...
def get_confident_rois(rois_dict, thresholds_per_class, translate_to_signpost_component=False):
    selected_dict = dict()
    for file_name, rois in rois_dict.items():
        rois_list = list()
        selected_dict[file_name] = rois_list
        for roi in rois:
            if translate_to_signpost_component:
                type_name = get_component_type_name(roi.type)
            else:
                type_name = get_roi_type_name(roi.type)

            if roi.detections[0].confidence > thresholds_per_class[type_name]:
...
def filter_rois_by_classes(selected_classes, all_rois_dict, as_sign_component=False):
    all_selected_rois = dict()
    for file_name, rois in all_rois_dict.items():
        selected_rois = list()
        for roi in rois:
            label, rect = get_rect_from_roi(roi)
            if as_sign_component:
                label = get_component_type_name(roi.type)
