...
class GeoSelector(AbstractMQConsumer):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.regions_to_process = set(config_api.get_config_param(MQ_Param.REGIONS_TO_PROCESS, config, ["US"]))
        self.default_output_queue = config_api.get_config_param(MQ_Param.MQ_OUTPUT_QUEUE_NAME, config)
    ...
    def consume_msg(self, input_message):
        ...
        # temp solution in order to process CA images with US model
        augmented_region = region if region != "CA" else "US"
